...
@login_required
def project_list_view(request: Any) -> HttpResponse:
    """Widok listy projektów (tylko główne, reszta w rekurencji)."""
    # Pobieramy tylko projekty 'root' (bez rodzica)
    root_projects = Project.objects.filter(user=request.user, parent_project__isnull=True).prefetch_related(
        "subprojects"
    )  # Optymalizacja zapytań

    return render(request, "projects/project_list.html", {"projects": root_projects})


@login_required
def project_detail_view(request: Any, pk: int) -> HttpResponse:
    ...
    projected_date = predictor.predict_completion_date(list(active_tasks))
...
@login_required
def project_create_view(request: Any) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        # Tworzymy projekt
        project = Project.objects.create(user=request.user, title=title, parent_project=parent, area=area)
    ...
    return render(
        request, "projects/project_form.html", {"all_projects": all_projects, "areas": areas, "all_tags": all_tags}
    )
