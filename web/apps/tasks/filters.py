...
class TaskFilter(django_filters.FilterSet):  # type: ignore
    ...
    status = django_filters.ChoiceFilter(
        choices=Task.StatusChoices.choices, label="Status", widget=forms.Select(attrs={"class": "form-select"})
    )
    context = django_filters.ModelChoiceFilter(
        queryset=Context.objects.all(), label="Kontekst", widget=forms.Select(attrs={"class": "form-select"})
    )
    ...
    area = django_filters.ModelChoiceFilter(
        queryset=Area.objects.all(), label="Obszar", widget=forms.Select(attrs={"class": "form-select"})
    )
