...
class GoogleCalendarAdapter(ICalendarProvider):
    def _fetch_from_google(self, service: Any, t_min: str, t_max: str) -> list[Any]:
        ...
        try:
            events_result = (
                service.events()
                .list(calendarId="primary", timeMin=t_min, timeMax=t_max, singleEvents=True, orderBy="startTime")
                .execute()
            )
            ...
            for event in events:
                ...
                fixed_events.append(
                    FixedEvent(
                        title=event.get("summary", "Bez tytułu"), start_time=start_dt, end_time=end_dt, is_work=True
                    )
                )
    ...
    def _get_service(self, user_id: int) -> Any:
    ...
    def get_events_range(self, user_id: int, start_date: Any, end_date: Any) -> list[Any]:
