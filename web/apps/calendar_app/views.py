...
def daily_view(request: Any) -> HttpResponse:
    ...
    work_windows = scheduler.calculate_free_windows(
        today,
        fixed_events,
        work_start=parse_time_string(profile.work_start_hour),
        work_end=parse_time_string(profile.work_end_hour),
    )

    work_schedule = scheduler.schedule_tasks(work_tasks, work_windows, now, user_profile=profile)
    ...
    personal_windows = scheduler.calculate_free_windows(
        today,
        fixed_events,
        work_start=parse_time_string(profile.personal_start_hour),
        work_end=parse_time_string(profile.personal_end_hour),
    )

    personal_schedule = scheduler.schedule_tasks(personal_tasks, personal_windows, now, user_profile=profile)
    ...
    for item in full_schedule:
        ...
        if hasattr(item.task, "area_color") and item.task.area_color:
            task_color = item.task.area_color
        elif hasattr(item.task, "area") and item.task.area and hasattr(item.task.area, "color"):
            task_color = item.task.area.color
        else:
            task_color = "#0d6efd" if not item.task.is_private else "#198754"
    ...
    timeline_items.sort(key=lambda x: x["start"])  # type: ignore
...
def weekly_view(request: Any) -> HttpResponse:
    ...
    milestones = Task.objects.filter(user=request.user, is_milestone=True, due_date__range=[start_of_week, end_of_week])
...
def monthly_view(request: Any) -> HttpResponse:
    ...
    # Cele
    goals = Goal.objects.filter(user=request.user, deadline__year=year, deadline__month=month)
    # Projekty
    projects = Project.objects.filter(user=request.user, deadline__year=year, deadline__month=month)
    # Kamienie Milowe (Zadania oznaczone jako milestone)
    milestones = Task.objects.filter(user=request.user, due_date__year=year, due_date__month=month, is_milestone=True)

    # 3. Zmapuj dane na dni miesiąca: { 15: [obj1, obj2], 20: [obj3] }
    events_by_day: dict[str, Any] = {}

    def add_event(day: Any, obj: Any, type: str, color: str) -> None:
    ...
    for g in goals:
        if g.deadline is not None:
            add_event(g.deadline.day, g, "Cel", "bg-danger")
    for p in projects:
        if p.deadline is not None:
            add_event(p.deadline.day, p, "Projekt", "bg-primary")
    for m in milestones:
        if m.due_date is not None:
            add_event(m.due_date.day, m, "Milestone", "bg-info")
